...
class PostApi {
  ...
  Future<dynamic> getPosts({int? take, int? page}) {
    return _client
        .get<dynamic>(
          '/post-api/posts',
          queryParameters: {if (take != null) 'take': take, if (page != null) 'page': page},
        )
        .then((value) => value.data);
  }

  Future<dynamic> createPost({required String channel, required String title, required String content}) {
    return _client
        .post<dynamic>('/post-api/post/create', data: {'channel': channel, 'title': title, 'content': content})
        .then((value) => value.data);
  }
  ...
  Future<dynamic> getSearchPosts({String query = '', int? take, int? page}) {
    return _client
        .get<dynamic>(
          ...
          queryParameters: {'query': query, if (take != null) 'take': take, if (page != null) 'page': page},
        )
        .then((value) => value.data);
  }
}
